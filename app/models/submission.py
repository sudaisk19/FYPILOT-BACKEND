...
class Submission(Base):
    ...
    supervisor_graded_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True))
    ...
    linked_announcement: Mapped[Optional["Announcement"]] = relationship(
        "Announcement", back_populates="submissions", foreign_keys=[linked_announcement_id]
    )
...
class SubmissionFile(Base):
    ...
    file_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), primary_key=True, default=uuid4)
    submission_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("submissions.submission_id", ondelete="CASCADE")
    )
