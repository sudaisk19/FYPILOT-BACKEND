...
class FileService:
    ...
    async def upload_file(
        ...
        try:
            ...
            # Get public URL
            public_url = supabase.storage.from_(FileService.BUCKET_NAME).get_public_url(storage_path)
    ...
    async def upload_multiple_files(
        ...
        for file in files:
            storage_key, file_name, size_bytes, mime_type = await FileService.upload_file(
                file, folder
            )
    ...
    def get_file_url(storage_key: str) -> str:
        ...
        """
        return supabase.storage.from_(FileService.BUCKET_NAME).get_public_url(storage_key)
    ...
    async def download_file(storage_key: str) -> bytes:
        ...
        """
        try:
            result = supabase.storage.from_(FileService.BUCKET_NAME).download(storage_key)
            return result
        except Exception as e:
            raise HTTPException(
                status_code=404,
