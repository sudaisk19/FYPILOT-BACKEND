...
def _build_response(announcement: Announcement) -> SubmissionAnnouncementResponse:
    ...
    return SubmissionAnnouncementResponse(
        ...
        total_marks=float(announcement.total_marks) if announcement.total_marks else None,
...
async def create_submission_announcement(
    ...
    dueDate: Optional[str] = Form(None, description="Due date in ISO format (YYYY-MM-DDTHH:MM:SSZ)"),
    total_marks: Optional[float] = Form(None, description="Total marks for the submission"),
    uploaded_files: Optional[List[UploadFile]] = File(default=None, description="Attachment files to upload"),
    file_types: Optional[str] = Form(None, description="Comma-separated file types for each uploaded file (Document or Template). E.g., 'Document,Template,Document'"),
    file_modules: Optional[str] = Form(None, description="Comma-separated module names for Template files (use empty string for Documents). E.g., ',Module A,' for 3 files where only 2nd is Template"),
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    ...
    if dueDate:
        try:
            ...
            raise HTTPException(status_code=400, detail="Invalid dueDate format. Use ISO format: YYYY-MM-DDTHH:MM:SSZ")
    ...
    # 3. Upload new files if provided (filter out empty/invalid files)
    valid_files = []
    valid_file_indices = []  # Track original indices for mapping to file_types/file_modules
    if uploaded_files:
    ...
    if valid_files:
        uploaded_file_data = await FileService.upload_multiple_files(valid_files, folder="announcements")
        for i, f_data in enumerate(uploaded_file_data):
            ...
            ftype_str = types_list[original_idx] if original_idx < len(types_list) else "Document"
            ftype = FileTypeEnum.Template if ftype_str.lower() == "template" else FileTypeEnum.Document
            
            # Determine module (only relevant for Template)
            fmodule = modules_list[original_idx] if original_idx < len(modules_list) and modules_list[original_idx] else None
...
async def edit_submission_announcement(
    ...
    description: Optional[str] = Form(None, description="Announcement description"),
    assignTo: Optional[str] = Form(None, description="Target audience: Students, Supervisors, or Both"),
    dueDate: Optional[str] = Form(None, description="Due date in ISO format (YYYY-MM-DDTHH:MM:SSZ)"),
    total_marks: Optional[str] = Form(None, description="Total marks for the submission"),
    keep_file_ids: Optional[str] = Form(None, description="Comma-separated file IDs to keep (e.g., 'uuid1,uuid2')"),
    uploaded_files: Optional[List[UploadFile]] = File(default=None, description="New attachment files to upload"),
    file_types: Optional[str] = Form(None, description="Comma-separated file types for each uploaded file (Document or Template)"),
    file_modules: Optional[str] = Form(None, description="Comma-separated module names for Template files (use empty string for Documents)"),
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    ...
    keep_file_ids = keep_file_ids.strip() if keep_file_ids and keep_file_ids.strip() else None
    file_types = file_types.strip() if file_types and file_types.strip() else None
    file_modules = file_modules.strip() if file_modules and file_modules.strip() else None
    ...
    if not announcement.is_submission_request:
        raise HTTPException(status_code=400, detail="This announcement is not a submission request")
    ...
    if body.assignTo is not None:
        # Remove old targets
        await db.execute(
            delete(AnnouncementTarget)
            .where(AnnouncementTarget.announcement_id == announcement_id)
        )
        # Add new targets
    ...
    if keep_file_ids is not None:  # If keep_file_ids is provided, sync files
    ...
    # 5. Upload new files if provided (filter out empty/invalid files)
    valid_files = []
    valid_file_indices = []  # Track original indices for mapping to file_types/file_modules
    if uploaded_files:
        for idx, f in enumerate(uploaded_files):
            # Check if it's actually an UploadFile (not an empty string from form data)
            if hasattr(f, 'filename') and f.filename and hasattr(f, 'size') and f.size and f.size > 0:
                valid_files.append(f)
                valid_file_indices.append(idx)
    ...
    for announcement in announcements:
        ...
        tasks_out.append(
            SubmissionTaskInfo(
                ...
                total_points=float(announcement.total_marks) if announcement.total_marks else None,
...
@router.get("/submission-tasks/{announcement_id}/submissions", response_model=GroupSubmissionsResponse)
async def get_submission_responses(
    announcement_id: UUID,
    search: Optional[str] = Query(None, description="Search by group name or FYP ID"),
    status_filter: Optional[str] = Query(None, description="Filter by status: submitted, missing, graded, returned"),
    page: int = Query(1, ge=1),
    per_page: int = Query(10, ge=1, le=50),
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
    ...
    submissions_result = await db.execute(
        select(Submission)
        .where(Submission.linked_announcement_id == announcement_id)
    )
    submissions_by_group = {sub.group_id: sub for sub in submissions_result.scalars().all()}
    ...
    if status_filter:
        ...
        filtered_submissions = [
            sub for sub in filtered_submissions
            if sub.status.lower() == status_lower
        ]
...
async def get_submission_evaluation(
    ...
    if submission.linked_announcement:
        total_marks = float(submission.linked_announcement.total_marks) if submission.linked_announcement.total_marks else None
    ...
    return SubmissionEvaluationResponse(
        ...
        supervisorMarks=float(submission.supervisor_marks) if submission.supervisor_marks else None,
